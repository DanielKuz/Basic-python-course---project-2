...
class Sprite:
    ...
    def collide(self, obj):
        if dist(self.pos, obj.get_position()) <= obj.get_radius() + self.radius:
            return True
        return False
...
def draw(canvas):
    ...
    # check for collison between ship and rocks
...
def process_sprite_group(canvas):
    rocks = rock_group.copy()
    for r in rocks:
        r.update()
        r.draw(canvas)
    itr_missile_group = list(missile_group)
    for m in itr_missile_group:
        if m.update() == True:
...
def group_group_collide(group1, group2):
    global score_number
    score_number = 0
    temp_group1 = list(group1)
    for m in temp_group1:
...
def rock_spawner():
    global a_rock
    if len(rock_group) < 25:
        rock_pos = [random.randrange(0, WIDTH), random.randrange(0, HEIGHT)]
        rock_vel = [random.random() * .6 - .3, random.random() * .6 - .3]
        rock_avel = random.random() * .2 - .1
        if dist(rock_pos, my_ship.get_position()) > 50:
